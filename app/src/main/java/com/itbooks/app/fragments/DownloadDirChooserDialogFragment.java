...
public final class DownloadDirChooserDialogFragment extends DirectoryChooserFragment {
	...
	private static final String EXTRAS_COPY_OR_MOVE = DownloadDirChooserDialogFragment.class.getName() + ".EXTRAS.copyOrMove";
	...
	public void onEvent( final OnDirectoryChosenEvent e ) {
		...
		File from = new File( App.Instance.getExternalFilesDir( Environment.DIRECTORY_DOWNLOADS ), download.getTargetName() );
	...
	public static DownloadDirChooserDialogFragment newInstance( Context cxt, int type, Download download, File externalFilesDir ) {
		...
		return (DownloadDirChooserDialogFragment) DownloadDirChooserDialogFragment.instantiate( cxt, DownloadDirChooserDialogFragment.class.getName(),
																								args
		);
	}
