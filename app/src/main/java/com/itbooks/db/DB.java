...
public final class DB {
	...
	Download getDownload( long downloadId ) {
		...
		try {
			RSBook book;
			while( c.moveToNext() ) {
				book = new RSBook( c.getString( c.getColumnIndex( DownloadsTbl.BOOK_NAME ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_AUTH ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_SIZE ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_PAGES ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_LINK ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_ISBN ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_YEAR ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_PUB ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_DESC ) ),
								   c.getString( c.getColumnIndex( DownloadsTbl.BOOK_COVER_URL ) )
				);
	 ...
	 *
	 * @param book
	 * 		A book that might have being downloaded.
	 *
	...
	public synchronized List<Download> getDownloads( RSBook book ) {
		...
		try {
			...
			c = mDB.query( DownloadsTbl.TABLE_NAME, null, whereClause, whereArgs, null, null, DownloadsTbl.EDIT_TIME + " DESC" );

			while( c.moveToNext() ) {
				Download item = new Download( new RSBook( c.getString( c.getColumnIndex( DownloadsTbl.BOOK_NAME ) ),
														  c.getString( c.getColumnIndex( DownloadsTbl.BOOK_AUTH ) ),
														  c.getString( c.getColumnIndex( DownloadsTbl.BOOK_SIZE ) ),
														  c.getString( c.getColumnIndex( DownloadsTbl.BOOK_PAGES ) ),
														  c.getString( c.getColumnIndex( DownloadsTbl.BOOK_LINK ) ),
	...
	public synchronized List<Download> getDownloads() {
		...
		List<Download> downloads = new ArrayList<>();
		try {
			c = mDB.query( DownloadsTbl.TABLE_NAME, null, null, null, null, null, DownloadsTbl.EDIT_TIME + " DESC" );
	 ...
	 *
	 * @param id
	 * 		The ident of a download entry. If id<0, delete all.
	 *
