...
final class DatabaseHelper extends SQLiteOpenHelper {
	...
	public void onCreate( SQLiteDatabase db ) {
		db.execSQL(
				DownloadsTbl.SQL_CREATE );//New table from v2, if user had never created bookmark-table or label-table, Android creates download-table for initialization.
	}
	...
	public void onUpgrade( SQLiteDatabase db, int oldVersion, int newVersion ) {
		if( oldVersion == 1 && newVersion == 2 ) {
			...
			db.execSQL(
					DownloadsTbl.SQL_CREATE );//New table for v2, if user had created bookmark-table or label-table, Android creates download-table for update.
