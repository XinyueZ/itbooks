...
public class SyncService extends Service implements ConnectionCallbacks, OnConnectionFailedListener {
	...
	private static DriveId getITBooksFolderId( GoogleApiClient client ) {
		...
		Query query = new Query.Builder().addFilter( Filters.eq( SearchableField.TRASHED, false ) ).addFilter(
				Filters.eq( SearchableField.TITLE, FOLDER_NAME ) ).addFilter( Filters.eq( SearchableField.MIME_TYPE, DriveFolder.MIME_TYPE ) )
				.build();
			...
			if( downloadTotal > 0 ) {
				for( Download download : downloads ) {
					...
					if( fileBufferResult.getStatus().isSuccess() ) {
						...
						for( Metadata metaData : metadataBuffer ) {
							if( !metaData.isFolder() && TextUtils.equals( download.getTargetName(), metaData.getTitle() ) ) {
								fileExist = true;
								break;
							}
						}
					...
					if( fileExist ) {
						...
						try {
							...
							if( driveContentsResult.getStatus().isSuccess() ) {
								...
								driverStream.write(
										IOUtils.toByteArray( FileUtils.openInputStream( new File( downloadsDir, download.getTargetName() ) ) ) );
								...
								MetadataChangeSet createdFileMeta = new MetadataChangeSet.Builder().setMimeType( MIME_TYPE ).setTitle(
										download.getTargetName() ).setCustomProperty( bookName, download.getName() ).setCustomProperty(
										bookAuthor, download.getAuthor() ).setCustomProperty( bookSize, download.getSize() ).setCustomProperty(
										bookPages, download.getPages() ).setCustomProperty( bookLink1, link1 ).setCustomProperty( bookLink2, link2 )
										.setCustomProperty(
												bookISBN, download.getISBN() ).setCustomProperty( bookYear, download.getYear() ).setCustomProperty(
												bookPublisher, download.getPublisher() ).setCustomProperty( bookCover1, s1 ).setCustomProperty(
												bookCover2, s2 ).setDescription( download.getDescription() ).build();
								DriveFileResult fileResult = itBooksFolder.createFile( client, createdFileMeta, contents ).await();
								if( fileResult.getStatus().isSuccess() ) {
									...
									if( newFileMeta.getStatus().isSuccess() ) {
										try {
											Bitmap image = Picasso.with( App.Instance ).load(
													Utils.uriStr2URI( download.getCoverUrl() ).toASCIIString() ).get();
											NotifyUtils.notifyWithBigImage( App.Instance, (int) System.currentTimeMillis(),
																			App.Instance.getString( R.string.application_name ),
																			App.Instance.getString( R.string.msg_push_success ) +
																			download.getTargetName(), android.R.drawable.stat_notify_sync, image,
											...
											NotifyUtils.notifyWithoutBigImage( App.Instance, (int) System.currentTimeMillis(),
																			   App.Instance.getString( R.string.application_name ),
																			   App.Instance.getString( R.string.msg_push_success ) +
																			   download.getTargetName(), android.R.drawable.stat_notify_sync,
																			   NotifyUtils.getDrive( App.Instance, itBooksFolder.getDriveId() )
											);
										}
									} else {
			...
			if( filesBufferResult.getStatus().isSuccess() ) {
				...
				for( Metadata metaData : metadataBuffer ) {
					...
					RSBook book = new RSBook( propertyKeyStringMap.get( bookName ), propertyKeyStringMap.get( bookAuthor ),
											  propertyKeyStringMap.get( bookSize ), propertyKeyStringMap.get( bookPages ), linkUrl,
											  propertyKeyStringMap.get( bookISBN ), propertyKeyStringMap.get( bookYear ),
											  propertyKeyStringMap.get( bookPublisher ), description, coverUrl
					);
					...
					if( downloads.size() == 0 ) {
						...
						File localFile = new File( App.Instance.getExternalFilesDir( Environment.DIRECTORY_DOWNLOADS ), metaData.getTitle() );
						try {
							...
							try {
								Bitmap image = Picasso.with( App.Instance ).load( Utils.uriStr2URI( download.getCoverUrl() ).toASCIIString() ).get();


								NotifyUtils.notifyWithBigImage( App.Instance, (int) System.currentTimeMillis(),
																App.Instance.getString( R.string.application_name ),
																App.Instance.getString( R.string.msg_pull_success ) + download.getTargetName(),
																android.R.drawable.stat_notify_sync, image,
																NotifyUtils.getPDFReader( App.Instance, to )
								);
	...
	private static void deleteFiles( GoogleApiClient client, long[] fileIDs, DriveFolder itBooksFolder ) {
		if( itBooksFolder != null ) {
			...
			for( long downloadId : fileIDs ) {
				...
				try {
					File localFile = new File( App.Instance.getExternalFilesDir( Environment.DIRECTORY_DOWNLOADS ), download.getTargetName() );
					if( localFile.exists() ) {
				...
				Query query = new Query.Builder().addFilter( Filters.eq( SearchableField.MIME_TYPE, MIME_TYPE ) ).addFilter(
						Filters.eq( SearchableField.TITLE, download.getTargetName() ) ).build();
				MetadataBufferResult fileBufferResult = itBooksFolder.queryChildren( client, query ).await();
				...
				for( Metadata metaData : fileMetadataBuffer ) {
					...
					if( status.isSuccess() ) {
						...
						NotifyUtils.notifyWithoutBigImage( App.Instance, NOTIFY_ERROR_DEL_ID, App.Instance.getString( R.string.application_name ),
	...
	private void establishGoogleDriver() {
		if( !TextUtils.isEmpty( Prefs.getInstance( App.Instance ).getGoogleId() ) && mGoogleApiClient == null ) {
			mGoogleApiClient = new GoogleApiClient.Builder( this ).addApi( Drive.API ).addScope( Drive.SCOPE_FILE ).addOnConnectionFailedListener(
		...
		if( !connectionResult.hasResolution() ) {
			switch( connectionResult.getErrorCode() ) {
				...
				case ConnectionResult.INVALID_ACCOUNT:
					NotifyUtils.notifyWithoutBigImage( App.Instance, NOTIFY_REQ_LOGIN, App.Instance.getString( R.string.application_name ),
													   App.Instance.getString( R.string.msg_sync_need_login ), android.R.drawable.stat_notify_error,
													   NotifyUtils.getGoogleLogin( App.Instance )
					);
					break;
			}
		}
